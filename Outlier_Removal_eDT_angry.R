...
# write.csv(out, "DT_angry_out_nooutliers.csv", row.names=F)
# write.csv(DT,"DT_angry_long_nooutliers.csv",row.names=F)
# write.csv(DT_wide_trim,"DT_angry_wide_nooutliers.csv",row.names=F)

###hokay, looks like it did work upon running an alternate way, but it's also almost time to go to the IRC, so I'll investigate more later###